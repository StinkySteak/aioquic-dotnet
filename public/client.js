...
async function readDatagrams(transport) {
    ...
    addToEventLog('Constructing decoder...');
    try {
        while (true) {
            addToEventLog('I`m waiting for message...');
            const { value, done } = await reader.read();
            addToEventLog('I received a message!');
            if (done) {
                addToEventLog('Done reading datagrams!');
                return;
            }
            addToEventLog('Datagram received (1): ' + value);
            let data = decoder.decode(value);
            addToEventLog('Datagram received (2): ' + data);
        }
    } catch (e) {
